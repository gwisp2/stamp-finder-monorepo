...
describe('Parsing encoded options results in the same options', () => {
  const check = (options: SearchOptions) => {
    ...
    test(`"${encodedOptions}"`, () => expect(SearchOptions.fromUrlSearchString(encodedOptions)).toStrictEqual(options));
  };
  ...
  check(
    SearchOptions.Default.applyChange({
      sort: StampSort.fromString('value-desc'),
      year: NumberRange.between(2010, 2013),
      category: 'cat',
    }),
  );
});
