...
export class RangeSelector extends React.Component<RangeSelectorProps, RangeSelectorState> {
    ...
    render() {
        return (<Form.Group className={this.props.className}>
            <Form className="range-selector-row mb-2">
                {this.props.label}
                <DropdownButton variant="custom-white" title={this.props.value.exact ? "Ровно" : "Между"} size="sm" className="ms-2">
            ...
            {!this.props.value.exact ? (
                ...
                <Form className="range-selector-row">
                    <Form.Control name="exactStr" type="number" value={this.state.startStr}
                        onChange={this.handleNumberChange} />
                </Form>
            )
            }
    ...
    private setExact(exact: boolean) {
        if (this.props.onChange !== undefined) {
            const newRange = exact ? NumberRange.exact(this.props.value.start) : NumberRange.between(this.props.value.start, this.props.value.end);
    ...
    private handleNumberChange(e: ChangeEvent<HTMLInputElement>) {
        ...
        if (this.props.onChange !== undefined) {
            const newRange = this.props.value.exact ?
                NumberRange.exact(newStart) : NumberRange.between(newStart, newEnd);
            this.props.onChange(newRange);
        }
    }
