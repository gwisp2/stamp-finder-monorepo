...
class SectionHeader(Section):
    def __init__(
        self,
        text: str,
        stamp_id_or_range: Optional[Union[int, Tuple[int, int]]],
        title: str,
    ):
        self.text = text
...
class PositionPageParser:
    @staticmethod
    def parse_stamp_entries(
        content: Union[str, bytes, List[Section]]
    ) -> List[StampBaseInfo]:
            ...
            if isinstance(section, SectionHeader):
                if section.stamp_id_or_range is not None:
                    if isinstance(section.stamp_id_or_range, int):
                        ...
                        for x in range(
                            section.stamp_id_or_range[0],
                            section.stamp_id_or_range[1] + 1,
                        ):
                ...
                if last_id:
                    ...
                    if info(last_id).value is None:
                        buy_offer = next(
                            (
                                o
                                for o in section.buy_offers
                                if o.ids == [last_id] and o.typ == "Чистый"
                            ),
                            None,
                        )
        ...
        for section_element in marka_main_div:
            ...
            if h:
                ...
                if "№" not in h.text:
                    ...
                    # Parse stamp ids
                    id_range_match = re.search(
                        r"^№\s*(\d+)[AА]?-(\d+)[AА]?[\s.]?(\s*.*$|$)", h.text.strip()
                    )
                    ...
                    if id_range_match:
                        ...
                        sections.append(
                            SectionHeader(
                                h.text,
                                (stamp_id_start, stamp_id_end),
                                id_range_match.group(3).strip(),
                            )
                        )
                    elif id_match:
                        stamp_id = int(id_match.group(1))
                        sections.append(
                            SectionHeader(h.text, stamp_id, id_match.group(2).strip())
                        )
                ...
                section_element.name == "h3"
                and section_element.text == "С этим покупают"
            ):
                # List of other stamps, ignore this
                pass
            else:
